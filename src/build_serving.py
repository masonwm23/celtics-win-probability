...
def build_game_payload(game_id: str, events: pd.DataFrame,
    ...
    return {
        ...
        "events": {
            ...
            # The four model inputs not already above, plus the two clocks the
            # model splits on, so the what-if panel can evaluate the saved
            # model in the browser instead of calling Python. The remaining
            # seven of the thirteen are either here already or derived from
            # them by web/lib/wp-model.js, using the same rule as
            # src/features.py.
            #
            # Seconds are rounded to hundredths, which is the resolution the
            # NBA clock is recorded at. Verified lossless: rounding to 2dp and
            # recomputing margin_per_minute_remaining from the rounded value
            # changes no prediction on any of the 308,975 rows. Everything
            # else here is a whole number.
